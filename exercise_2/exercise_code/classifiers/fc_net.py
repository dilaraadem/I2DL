...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        ############################################################################
        W1 = np.random.randn(input_dim, hidden_dim) * weight_scale # N x H
        W2 = np.random.randn(hidden_dim, num_classes) * weight_scale # H x C
    ...
    def loss(self, X, y=None):
        ...
        # compute the loss
        reg_loss = np.sum(np.square(self.params['W1'])) + np.sum(np.square(self.params['W2']))
        loss = dataloss + 0.5 * self.reg * reg_loss
        
        # Compute grads
        dout1, dw2, db2 = affine_backward(dscores, cache2)
        dx, dw1, db1 = affine_relu_backward(dout1, cache1)
                
        # add to dict + regularization
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(1, self.num_layers, 1):
            cur_dim = hidden_dims[i - 1]
            self.params['W' + str(i)] = np.random.randn(last_dim, cur_dim) * weight_scale
            self.params['b' + str(i)] = np.zeros(cur_dim)
            last_dim = cur_dim
        
        i = self.num_layers
        self.params[("W" + str(i))] = np.random.randn(last_dim, num_classes) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers, 1):
            ...
            aff_out, aff_cache = affine_forward(last_x, self.params["W"+str(i)], self.params["b"+str(i)])
            affine_cache[i] = aff_cache
            # relu_forward(out)
            rel_out, rel_cache = relu_forward(aff_out)
            relu_cache[i] = rel_cache
            last_x = rel_out
        #last layer Wn, bn
        i = self.num_layers
        scores, last_cache = affine_forward(last_x, self.params["W" + str(i)], self.params["b" + str(i)])

...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        # W2(max(0,W1(X)))
        f = X.dot(W1) + b1 # X*W +b
        r = np.maximum(0,f) #ReLU activation, r is hidden layer
        scores = r.dot(W2) + b2
        ...
        expo_probs = np.exp(scores)
        sum_probs = np.sum(expo_probs, axis=1, keepdims=True)
        p = expo_probs / sum_probs #calculate probability estimates
    
        log_probs = -np.log(p[range(N), y])
        loss = np.sum(log_probs) #calculate loss
    
        loss /= float(N) # get average of loss
        loss += 0.5 * reg * (np.sum(W1 * W1) + np.sum(W2 * W2)) # do regularization
        ...
        dW2 = np.dot(r.T, grad_scores) #(NxC)
        db2 = np.sum(grad_scores, axis=0)
        
        #backprop into hidden layer
        dh = np.dot(grad_scores, W2.T)
        dh[r <= 0] = 0 #minimum should be 0. backprop the ReLU non-linearity
...
def neuralnetwork_hyperparameter_tuning(X_train, y_train, X_val, y_val):
    ...
    hidden_size = [60, 80, 90, 100, 120, 150, 200, 500]
    input_size = 32*32*3
    num_class = 10
    learning_rate = [1e-3, 1.5e-3, 2e-3, 3e-3]
    regular_vals = [0.01, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3]
    best_acc = -1
    ...
    for hs in hidden_size:
        for lr in learning_rate:
            for reg in regular_vals:
                ...
                print('hs %d lr %e reg %e train accuracy: %f val accuracy: %f' % (hs,
                    lr, reg, train_acc, val_acc))
        
    print('best validation accuracy achieved during validation: %f' % best_acc)

...
class FacialKeypointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_name = self.key_pts_frame.loc[idx]['Image']
        image = np.array([int(item) for item in image_name.split()]).reshape(-1,96,96)
        
        key_cols = list(self.key_pts_frame.columns)[:-1] #omits the Image column
        keypoints = self.key_pts_frame.iloc[idx][key_cols].values.reshape((15, 2)).astype(np.float32)
